...
_SCHEMA = (
    '{"core": "80-180字核心判断,先给结论", '
    '"points": [{"label": "维度名,如 项目定位/关键约束/推进重点", "text": "一句话"}], '
    '"actions": ["下一步动作,一句一条"], '
    '"questions": ["待确认问题,一句一条"], '
...
def build_judgment_prompt(
    ...
    """产出 (system, user, response_format) 给 DeepSeek json mode。文风内嵌。"""
    system = (
        PROSE_STYLE
        + "\n\n你是建筑设计前期的项目助手。只基于给定材料判断;材料不足就在 questions 里列出待确认,"
        "不要臆测、不要编造来源。必须返回严格 JSON(不要任何额外解释或代码围栏)。\n"
        "JSON 结构:\n" + _SCHEMA + "\n"
        "约束:core 80-180字且先给结论;points 3-6 条、每条 label+一句 text;"
        "actions/questions 各 0-5 条、每条一句;detail 放完整内容(可用 markdown 小标题/要点)。"
    )
...
def run_structured(
    *, project_name: str, instruction: str, context: str, user_extra: str,
    ...
    answer = llm.chat_completion(
        ...
        api_key=api_key, base_url=base_url, model=model, response_format=fmt, timeout=120.0,
    )
